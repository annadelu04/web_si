...
const Login = () => {
  // Accesso alle funzioni globali dell'app (login status, user data)
  ...
  // Stato specifico per la registrazione: Ruolo dell'utente
    ...
    try {
      ...
      if (state === "Sign Up") {
        ...
        const { data } = await axios.post(backendUrl + "/api/auth/register", {
          ...
          email,
          password,
          userType, // Fondamentale per creare il tipo di utente corretto
        });
        ...
        if (data.success) {
          ...
          if (data.user && data.user.tipo_utente === 'bambino') {
            navigate('/child-select'); // I bambini scelgono il profilo avatar
          } else {
            navigate('/profile');      // Gli adulti vanno alla dashboard
          }
        } else {
          toast.error(data.message);
        }
